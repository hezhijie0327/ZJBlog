...
| 项目 | 安排 |
|---|---|
| **日期** | {YYYY.MM.D（周几）— MM.DD（周几）· N 天 N-1 晚 · 人群} |
| **路线** | {出发地} → {A（住 N-1 晚）} → {B（1 天）} → {出发地} |
| **大交通** | ✅ {车次 1} · ✅ {车次 2} · ✅ {车次 3}（时刻见大交通节） |
| **住宿** | ✅ {酒店}，{区间} ¥{实付} —— {位置红利一句} |
| **门票** | ✅ {票种} ¥{实付/双人}：{包含项 + 关键限次规则} |
| **预算** | 已落定 **¥{合计}**（{人数}）；全包预估 ¥{区间}（不含购物） |
| **还差 {N} 件事** | {待办 1}；{待办 2} |

> [!info] 数据口径
> 均于 {YYYY-MM-DD} 核实落定；演出场次与营业时间以出行当日官方公告为准。{订票背景一句，如"返程最高峰当日 12:00 起售——开售当天全中"}。
...
```geojson
{
  ...
  "features": [
    { "type": "Feature", "properties": { "name": "{站点}" }, "geometry": { "type": "Point", "coordinates": [lng, lat] } },
    { "type": "Feature", "properties": { "name": "{去程车次}" }, "geometry": { "type": "LineString", "coordinates": [[lng, lat], [lng, lat]] } }
  ]
}
```
